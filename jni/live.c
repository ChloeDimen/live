#include "com_dimen_jni_PusherNative.h"
 ...
 */
JNIEXPORT void JNICALL Java_com_dimen_jni_PusherNative_startPush(JNIEnv *env,
		jobject jobj, jstring url_jstr) {
	//初始化操作
 ...
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_dimen_jni_PusherNative_stopPush(JNIEnv *env,
		jobject jobj) {
	free(rtmp_path);
}
 ...
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_com_dimen_jni_PusherNative_realease(JNIEnv *env,
		jobject jobj) {
...
void add_264_sequence_header(unsigned char *pps, unsigned char *sps,
	...
	int body_size = 16 + sps_len + pps_len; //按照h264标准配置sps和pps，共使用了16个字节
	...
	//0x04表示Ping包，0x08为audio，0x09为video
 ...
 * Signature: ([B)V
 */
JNIEXPORT void JNICALL Java_com_dimen_jni_PusherNative_fireVideo(JNIEnv * env,
		jobject jobj, jbyteArray buffer) {
	//视频数据转成yuv420p
	//NV21->yuv420p
	...
	//预览画面，帧分为关键帧和普通帧，为了提高画面的纠错率，关键帧应该包含sps和pps数据
	//遍历NALU数组，根据NALU类型判断
	int sps_len, pps_len;
	unsigned char sps[100];
	unsigned char pps[100];
	memset(sps, 0, 100); //清空sps
 ...
 * Signature: ([BI)V
 */
JNIEXPORT void JNICALL Java_com_dimen_jni_PusherNative_fireAudio(JNIEnv *env,
		jobject jobj, jbyteArray array, jint len) {
}
